...
public class HoldableCloseTestThread {
...
//        	SelectThroughputPEC(conn, true, 10000);
        	
        	Thread per;
        	int threadcount = 30;
        	for (int i=0; i < threadcount; i++) {
        		per = new Thread(new myRunnablePERC(1, conn, true));
        		per.start();
        	...
        	for (;;) {
        		for (int i=0; i < threadcount; i++) {
            		pesc = new Thread(new myRunnablePESC(1, conn, true, pescCount));
            		pesc.start();
