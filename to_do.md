...
## 0-1. 구현 상태 (2026-07-08) — 방식 B: 클라이언트 전용(프로토콜/서버 변경 없음)

최종 채택: **방식 B = A(holdable cursor_close 지연 + 재실행 의존) + 커밋 flush.** JDBC 클라이언트만 수정하고 **서버·프로토콜은 변경하지 않는다.** (앞서 검토했던 C/V13 배치 방식은 되돌렸다 — 서버 `cas_protocol.h`/`cas_function.c`는 원복.)

**설계 근거 — 재실행 전 cursor_close는 원래 중복이었다 (검증 완료):** 서버 `ux_execute`(및 `_all`/`_call`/`_batch`)는 진입 즉시 `hm_qresult_end(srv_handle, FALSE)`로 이전 결과를 (holdable 포함) 해제한다(`cas_execute.c:1010` 등). `hm_qresult_end`는 `ux_cursor_close`와 **같은 `hm_free_result → db_query_end`** 를 호출하고 `num_holdable_results`도 동일하게 감소시킨다(`copied` 가드는 죽은 필드라 항상 해제). 즉 클라이언트가 재실행 앞에서 보내던 cursor_close는 서버가 어차피 하는 일과 **중복**이었고, 안 보내도 이전 결과는 재실행이 해제한다. 이건 **버전 의존성이 없다**(원래부터 그런 동작) → 서버/프로토콜 변경 불필요.

**동작 흐름:**
- `rs.close()`(holdable) → 즉시 전송 대신 커넥션 `deferred_cursor_close_handle`에 serverHandler를 mark (왕복 0).
- 이전 결과 해제 경로: ① **재실행**(같은 핸들) → 서버 `hm_qresult_end`가 해제(예약은 무효 → 목록에서 제거), ② **statement close**(`hm_srv_handle_free`), ③ **연결 종료**(강제 rollback+전량 해제).
- **커밋 flush:** 명시적 commit 직전, 남은 예약을 **개별 `CURSOR_CLOSE`로 flush**(비어있으면 무왕복). rollback은 전송 없이 discard(서버가 전부 해제).
- **안전밸브:** 목록이 `DEFERRED_CURSOR_CLOSE_MAX=32` 도달 시 개별 flush(주로 autocommit-다핸들 누적 방지).
- **핸들 ID 재사용 안전(§7):** 재실행(execute/batch)·statement close 시 serverHandler 제거, rollback·소켓 종료 시 clear → stale close가 재사용 핸들의 새 결과를 닫는 일 없음.

**효과 / 비용:**
- autocommit 포인트-셀렉트(PreparedStatement 재사용): 매 execute가 직전 예약을 무효화 → **CURSOR_CLOSE 왕복 소거(2→1)**, non-holdable 기준선 도달. 서버/프로토콜 무변경으로 달성.
- **비용은 "동시에 열린 holdable 커서 수"에 비례** — statement당 결과 1개(JDBC 규칙)이므로 **핸들 1개면 최대 1개만 잠깐 남고**(재실행/close에서 즉시 해제) 사실상 무비용. 누적(→ `num_holdable_results` 상승 → idle 시 CAS 리밸런싱 지연)은 **핸들 여러 개**를 닫되 재실행/close 안 할 때만 의미. 커밋 flush + MAX가 이를 억제.

**변경 파일 (JDBC만):**
| 파일 | 변경 |
|---|---|
| `jci/UConnection.java` | `deferred_cursor_close_handle` + `add`/`remove`/`clear`/`flushDeferredCursorClose`(개별 CURSOR_CLOSE, V2 함수코드 대응), socket close 시 clear. **V13/CAS_PROTOCOL_VERSION bump·supportsBatchedCursorClose 제거(원복)** |
| `jci/UStatement.java` | `closeCursor`는 무조건 defer(holdable 전용 경로), `executeInternal`·`executeBatchInternal` 재실행 시 remove, `close()` 시 remove |
| `jci/UClientSideConnection.java` | `endTransaction`: commit flush / rollback clear |
| (서버) `cas_protocol.h`, `cas_function.c` | **원복 완료 — 변경 없음** |

**검증:** JDBC 전체 트리 `javac` 컴파일 성공(126개, exit 0). 서버 2파일 `git diff` 없음(정확 원복). `[JDBC-Driver]` 로그(`JdbcDriverLog=true`)로 flush 시점/건수 관찰 가능.

**후속:**
- 서버 변경이 없으므로 **모든 브로커 버전에서 즉시 동작**(별도 빌드 불필요). 실측만 회원님 환경에서.
- **CCI(C 클라이언트)** 는 범위 제외 — 동일 패턴(지연 목록 + 재실행/close 시 제거 + 커밋 flush) 복제 필요.
